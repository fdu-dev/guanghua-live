...
public class EntityDAO<T, PK extends Serializable> extends HibernateDaoSupport {
	...
	public Long countByCriteria(final Class<T> entityClass,
			final Map<String, Object> properties){
		Long number = (long) getHibernateTemplate().execute(new HibernateCallback<Long>() {
	...
	public List<Map> findBySql(final String sql){
		List<Map> list = (List<Map>) getHibernateTemplate().execute(new HibernateCallback<List>() {
