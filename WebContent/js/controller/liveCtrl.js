/**
 * Created by HF Q on 2016/4/29.
 */
var server = window.localStorage ? localStorage.getItem("serverAddress") : Cookie.read("serverAddress");
angular.module('liveModule',[])
.controller('LiveCtrl',['$scope','$routeParams','$http',function($scope,$routeParams,$http){
    //如果你不需要某项设置，可以直接删除，注意var flashvars的最后一个值后面不能有逗号
    //$scope.videoTitle = "直播测试";
    var liveItemUrl = server+"videoGet?vid="+$routeParams.vid;

//    var mySrc = 'rtmp://10.108.81.231:1935/myapp/mystream';
//    var mySrc = 'http://movie.ks.js.cn/flv/other/2014/06/20-2.flv';
    $http.get(liveItemUrl)
        .success(function(response){
//            alert(JSON.stringify(response));
            $scope.video = response.video;
//            alert(JSON.stringify(response.video));
//            alert($scope.video.url);
//            alert(mySrc==$scope.video.url);
            var flashvars={
                f:$scope.video.url,//视频地址
//            	f:mySrc,
...
//            var video=['http://movie.ks.js.cn/flv/other/2014/06/20-2.mp4->video/mp4'];
            var video=[$scope.video.url];
            CKobject.embed('components/ckplayer6.7/ckplayer/ckplayer.swf','a1','ckplayer_a1','100%','100%',false,flashvars,video,params);
